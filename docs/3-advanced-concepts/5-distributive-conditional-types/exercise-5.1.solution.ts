...
type Flatten<T extends readonly unknown[]> = T[number];
...
// type A = "video" | "text" extends "video" ? true : false;

// type Same = Content['type'];

type Content =
  | { type: "text"; content: string }
  | { type: "image"; url: string }
  | { type: "video"; duration: number };
type Image = ExtractTypeByKey<Content, 'type', 'image'>;
//   ^?
